...
function Tables({ backEndServerUrl }) {
    ...
    const handleSubmit = (event) => {
        ...
        if (FormValidation() === true) {
            ...
            // setting initial formData.reservation_id to null triggers React warning, hence this if statement
            if (formData.reservation_id === "") {
              axios.post(createTableUrl, {data: {...formData, reservation_id: null}}, config);
            } else {
              axios.post(createTableUrl, {data: formData}, config);
            }
            // resetting the forms
