  ...
  describe("/dashboard page", () => {
    ...
    describe("clicking the reservation cancel button", () => {
      ...
      test("then clicking cancel makes no changes", async () => {
        ...
        const cancelButton = await page.$(cancelButtonSelector);
        /*
        [test:e2e:run       ]     button containing cancel not found.
        [test:e2e:run       ]
        [test:e2e:run       ]       115 |
        [test:e2e:run       ]       116 |         if (!cancelButton) {
        [test:e2e:run       ]     > 117 |           throw new Error("button containing cancel not found.");
        [test:e2e:run       ]           |                 ^
        [test:e2e:run       ]       118 |         }
        [test:e2e:run       ]       119 |
        [test:e2e:run       ]       120 |         page.on("dialog", async (dialog) => {
        [test:e2e:run       ]
        [test:e2e:run       ]       at Object.<anonymous> (us-08-change-existing-reservation.test.js:117:17)
        [test:e2e:run       ]           at runMicrotasks (<anonymous>)
        */
        if (!cancelButton) {
